...
class CollectorMethodVisitor(var sourceClassName: String?, var injectMethod: InjectMethod): MethodVisitor(ASM5) {

    override fun visitAnnotation(desc: String?, visible: Boolean): AnnotationVisitor? {
        if (CollectorAnnotationUtil.shouldCollectorSource(desc)) {
            LogUtil.debug("start visit annotation.,...")
            return CollectorAnnotationVisitor(sourceClassName, injectMethod.apply {
